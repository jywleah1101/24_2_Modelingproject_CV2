...
class Generator_3(nn.Module):
    ...
    # x_f0 : 원본 화자의 음성 데이터 + f0
    # x_org : 원본 화자의 음성 데이터
    # c_trg : 타겟 화자의 임베딩 벡터
    def forward(self, x_f0, x_org, c_trg, type_return):
        if type_return : 
            ...
            #리듬이랑 pitch반환
            return code_exp_1, code_exp_2, code_exp_3
        #변수명_변환(걍 가독성)
        encoder_outputs= x_f0
    ...
    # code_exp_1 : content vector
    # code_exp_2 : rhythm vector
    # code_exp_3 : pitch vector
    # c_trg : 바꿔주고자 하는 임베딩 벡터
    def meloutput(self, code_exp_1, code_exp_2, code_exp_3, c_trg):

        # 현재 사용 중인 장치를 확인 (GPU가 있으면 GPU, 없으면 CPU 사용)
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        # 모든 텐서를 동일한 장치로 이동
        code_exp_1 = code_exp_1.to(device)
        code_exp_2 = code_exp_2.to(device)
        code_exp_3 = code_exp_3.to(device)
        c_trg = c_trg.to(device)

        x_1 = code_exp_1.transpose(2,1)
